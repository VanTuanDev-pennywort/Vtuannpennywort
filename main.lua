...
-- ─── Centella Shim: replaces Obsidian Library ────────────────────
local function _MakeToggle(default)
    local t = {Value = default or false, _cbs = {}}
    function t:SetValue(v) self.Value = v for _,cb in ipairs(self._cbs) do pcall(cb,v) end end
    function t:OnChanged(cb) table.insert(self._cbs, cb) end
    return t
end
local function _MakeOption(default)
    local o = {Value = default, _cbs = {}, _clickcbs = {}}
    function o:SetValue(v) self.Value = v for _,cb in ipairs(self._cbs) do pcall(cb,v) end end
    function o:SetValues(v) end
    function o:GetValue() return self.Value end
    function o:OnChanged(cb) table.insert(self._cbs, cb) end
    function o:OnClick(cb) table.insert(self._clickcbs, cb) end
    function o:SetText(_) end
    function o:SetVisible(_) end
    return o
end
local function _MakeLabel()
    return {SetText=function()end, SetVisible=function()end}
end

local Toggles = {}
local Options = {}
local _toggleNames = {
    "MobFarm","AllMobFarm","LevelFarm","BossesFarm","AllBossesFarm",
    "AutoSummon","SummonBossFarm","PityBossFarm","AltBossFarm",
    "ObserHaki","ArmHaki","ConquerorHaki","AutoM1","KillAura",
    "AutoSkill","AutoCombo","SwitchWeapon","AutoStats","AutoRollStats",
    "AutoAscend","AutoTrait","AutoRace","AutoClan","AutoSpec",
    "AutoSkillTree","ArtifactMilestone","AutoEnchant","AutoEnchantAll",
    "AutoBlessing","AutoBlessingAll","ArtifactLock","ArtifactDelete",
    "ArtifactUpgrade","AutoDungeon","AutoInfiniteTower","AutoMerchant",
    "AutoChest","AutoCraftItem","AutoQuestline","AntiKnockback",
    "AntiAFK","AutoReconnect","FPSBoost","FPSBoost_AF","Noclip",
    "InstaKill","IslandTP","SendWebhook","AutoKick","TPW","WS","JP",
    "HH","Grav","FOV","Zoom","LimitFPS","Fullbright","NoFog",
    "OverrideTime","Disable3DRender","NoGameplayPaused","InstantPP",
    "AntiKick","AutoTitle","AutoRune","AutoServerhop","AutoDeleteNotif",
    "OnlyTarget","AutoSkill_BossOnly","ComboBossOnly","UpgradeStage",
    "DeleteUnlock","ArtifactEquip","AutoDiff","AutoReplay",
    "DungeonAutofarm","ReqTrade","ReqTradeAccept","AutoAccept",
    "PingUser","AutoShowUI","DungeonQuest","HogyokuQuest",
}
for _, n in ipairs(_toggleNames) do Toggles[n] = _MakeToggle(false) end
Toggles.AntiAFK.Value   = true
Toggles.SwitchWeapon.Value = true
Toggles.IslandTP.Value  = true

local _optionNames = {
    "SelectedMob","SelectedBosses","SelectedSummon","SelectedSummonDiff",
    "SelectedOtherSummon","SelectedOtherSummonDiff","SelectedBuildPity",
    "SelectedUsePity","SelectedPityDiff","SelectedAltBoss","SelectedAltDiff",
    "SelectedWeaponType","SwitchWeaponCD","IslandTPCD","TargetTPCD",
    "TargetDistTP","M1Speed","SelectedMovementType","SelectedFarmType",
    "Distance","TweenSpeed","InstaKillHP","InstaKillMinHP","InstaKillType",
    "KillAuraCD","KillAuraRange","SelectedSkills","AutoSkillType",
    "AutoSkill_BossHP","ComboPattern","ComboMode","WebhookURL","UID",
    "SelectedData","SelectedItemRarity","WebhookDelay",
    "SelectedStats","SelectedGemStats","SelectedRank","StatsRollCD",
    "RollCD","SelectedTrait","SelectedRace","SelectedClan","SelectedSpec",
    "SelectedPassive","SpecRollCD","UpgradeLimit","Up_MS",
    "Lock_Type","Lock_Set","Lock_MS","Lock_SS","Lock_MinSS",
    "Del_Type","Del_Set","Del_MS_Helmet","Del_MS_Gloves","Del_MS_Body",
    "Del_MS_Boots","Del_SS","Del_MinSS","Eq_Type","Eq_MS","Eq_SS",
    "SelectedDungeon","SelectedDiff","SelectedMerchantItems",
    "SelectedDungeonMerchantItems","SelectedValentineMerchantItems",
    "SelectedTowerMerchantItems","SelectedBossRushMerchantItems",
    "SelectedChests","SelectedCraftItems","SelectedQuestline",
    "SelectedQuestline_Player","SelectedQuestline_DMGTaken",
    "SelectedTradePlr","SelectedTradeItems","SelectedIsland",
    "SelectedQuestNPC","SelectedMiscNPC","SelectedMiscAllNPC",
    "SelectedMovesetNPC","SelectedMasteryNPC","SelectedKickType",
    "DefaultTitle","Title_Mob","Title_Boss","Title_BossHP","Title_Combo",
    "Title_BossHPAmt","DefaultRune","Rune_Mob","Rune_Boss","Rune_BossHP",
    "Rune_Combo","Rune_BossHPAmt","DefaultBuild","Build_Mob","Build_Boss",
    "Build_BossHP","Build_Combo","Build_BossHPAmt",
    "SelectedPriority_1","SelectedPriority_2","SelectedPriority_3",
    "SelectedPriority_4","SelectedPriority_5","SelectedPriority_6",
    "SelectedPriority_7","SelectedPriority_8","SelectedPriority_9",
}
for _, n in ipairs(_optionNames) do Options[n] = _MakeOption("") end
Options.SelectedMovementType.Value = "Tween"
Options.SelectedFarmType.Value     = "Behind"
Options.Distance.Value             = 12
Options.TweenSpeed.Value           = 160
Options.IslandTPCD.Value           = 0.67
Options.M1Speed.Value              = 0
Options.InstaKillHP.Value          = 90
Options.InstaKillMinHP.Value       = "100000"
Options.InstaKillType.Value        = "V1"
Options.KillAuraCD.Value           = 0.1
Options.KillAuraRange.Value        = 200
Options.AutoSkillType.Value        = "Normal"
Options.ComboPattern.Value         = "Z > X > C > V > F"
Options.ComboMode.Value            = "Normal"
Options.WebhookDelay.Value         = 5
Options.RollCD.Value               = 0.3
Options.StatsRollCD.Value          = 0.1
Options.SpecRollCD.Value           = 0.1
Options.UpgradeLimit.Value         = 0
Options.Lock_MinSS.Value           = 0
Options.Del_MinSS.Value            = 0
Options.SwitchWeaponCD.Value       = 4
Options.TargetDistTP.Value         = 0
Options.TargetTPCD.Value           = 0
Options.AutoSkill_BossHP.Value     = 100
Options.SelectedPriority_1.Value   = "Boss"
Options.SelectedPriority_2.Value   = "Pity Boss"
Options.SelectedPriority_3.Value   = "Summon [Other]"
Options.SelectedPriority_4.Value   = "Summon"
Options.SelectedPriority_5.Value   = "Level Farm"
Options.SelectedPriority_6.Value   = "All Mob Farm"
Options.SelectedPriority_7.Value   = "Mob"
Options.SelectedPriority_8.Value   = "Merchant"
Options.SelectedPriority_9.Value   = "Alt Help"
Options.SelectedSummonDiff.Value   = "Normal"
Options.SelectedOtherSummonDiff.Value = "Normal"
Options.SelectedPityDiff.Value     = "Normal"
Options.SelectedAltDiff.Value      = "Normal"
Options.SelectedKickType.Value     = {Mod = true}

local _WSO  = _MakeOption(16)
local _JPO  = _MakeOption(50)
local _HHO  = _MakeOption(2)
local _GrO  = _MakeOption(196)
local _FVO  = _MakeOption(70)
local _ZoO  = _MakeOption(128)
local _LFPO = _MakeOption(60)
local _OTO  = _MakeOption(12)
local _TPWO = _MakeOption(1)
Options.WSValue           = _WSO
Options.JPValue           = _JPO
Options.HHValue           = _HHO
Options.GravValue         = _GrO
Options.FOVValue          = _FVO
Options.ZoomValue         = _ZoO
Options.LimitFPSValue     = _LFPO
Options.OverrideTimeValue = _OTO
Options.TPWValue          = _TPWO
Options.PanicKeybind      = _MakeOption("P")
Options.MenuKeybind       = _MakeOption("U")
Options.PanicKeybind.OnClick = function(self, cb)
    self._clickcbs = self._clickcbs or {}
    table.insert(self._clickcbs, cb)
end

local Library = {Unloaded = false, Options = Options, Toggles = Toggles}
function Library:Notify(msg, dur)
    if _G.CENTELLA_NOTIFY then _G.CENTELLA_NOTIFY(tostring(msg), tonumber(dur) or 3) end
end
...
-- Stub UI labels (no-op since we have Centella UI)
local TierLabel, TimeLabel, StatsLabel, SpecPassiveLabel    = _MakeLabel(),_MakeLabel(),_MakeLabel(),_MakeLabel()
local ArtifactLabel, DustLabel                              = _MakeLabel(),_MakeLabel()
local InvLabel_Helmet, InvLabel_Gloves                      = _MakeLabel(),_MakeLabel()
local InvLabel_Body, InvLabel_Boots                         = _MakeLabel(),_MakeLabel()
local MerchantTimerLabel, PityLabel                         = _MakeLabel(),_MakeLabel()
local DungeonCount, ValentineLabel, ValentinePriceLabel     = _MakeLabel(),_MakeLabel(),_MakeLabel()

-- Expose to getgenv so CentellaUI can read/write
getgenv().CENTELLA_TOGGLES = Toggles
getgenv().CENTELLA_OPTIONS  = Options
getgenv().CENTELLA_LIB      = Library
-- AddSliderToggle stub (used in will.txt player section)
function AddSliderToggle(cfg)
    local t = Toggles[cfg.Id] or _MakeToggle(cfg.DefaultToggle or false)
    Toggles[cfg.Id] = t
    local s = Options[cfg.Id.."Value"] or _MakeOption(cfg.Default or 0)
    Options[cfg.Id.."Value"] = s
    t:OnChanged(function(v) s:SetVisible(v) end)
    return t, s
end
...
-- ═══════════════════════════════════════════
--  SP EXTRAS: Dungeon Quest + Hogyoku Quest
-- ═══════════════════════════════════════════
local SP_DungeonStep = 0
local SP_DungeonCollected = {}
local SP_HogyokuStep = 0
local SP_HogyokuCollected = {}
local SP_DungeonPieceIslands = {"Starter","Jungle","Desert","Snow","Shibuya","HuecoMundo"}
local SP_HogyokuFragmentIslands = {"Snow","Shibuya","HuecoMundo","Shinjuku","Slime","Judgement"}
local function SP_WaitChar(t) local d=tick()+(t or 15) while tick()<d do local c=Plr.Character if c and c:FindFirstChild("HumanoidRootPart") then local hm=c:FindFirstChildOfClass("Humanoid") if hm and hm.Health>0 then return true end end task.wait(0.15) end return false end
local function SP_ForceTP(p) pcall(function() RS.Remotes.TeleportToPortal:FireServer(p) end) task.wait(1.5) end
local function SP_ClickYes() local pg=Plr:FindFirstChild("PlayerGui") if not pg then return end for _,d in ipairs(pg:GetDescendants()) do if d:IsA("TextButton") then local t="" pcall(function() t=(d.Text or ""):lower() end) if t=="yes" or t=="confirm" then pcall(function() d.MouseButton1Click:Fire() end) end end end end
local function SP_FindDungeonPiece(island) local il=island:lower() for _,desc in ipairs(workspace:GetDescendants()) do if desc.Name=="DungeonPuzzlePiece" and desc:IsA("BasePart") then local a=desc.Parent while a and a~=workspace do if a.Name:lower():find(il,1,true) then return desc end a=a.Parent end end end return nil end
local function SP_PieceHasPrompt(p) if not p then return false end if p:FindFirstChildOfClass("ProximityPrompt") then return true end for _,c in ipairs(p:GetDescendants()) do if c:IsA("ProximityPrompt") then return true end end return false end
local function SP_CollectPiece(piece) local hrp=Plr.Character and Plr.Character:FindFirstChild("HumanoidRootPart") if hrp then hrp.CFrame=CFrame.new(piece.Position+Vector3.new(0,2,0)) end task.wait(0.15) local pp=piece:FindFirstChildOfClass("ProximityPrompt") if not pp then for _,c in ipairs(piece:GetDescendants()) do if c:IsA("ProximityPrompt") then pp=c break end end end if pp then pcall(function() fireproximityprompt(pp) end) task.wait(0.2) pcall(function() fireproximityprompt(pp) end) end end
local function SP_FindHogyokuFrag(n) local t="hogyoku fragment #"..n for _,d in ipairs(workspace:GetDescendants()) do if d:IsA("ProximityPrompt") and (d.ObjectText or ""):lower()==t then return d end end return nil end
local function SP_DoDungeonTick()
    if not Toggles.DungeonQuest.Value then return end
    local step=SP_DungeonStep
    if step<1 then
        pcall(function() RS.RemoteEvents.QuestAbandon:FireServer("DungeonUnlock") end)
        task.wait(0.2)
        pcall(function() RS.RemoteEvents.QuestAccept:FireServer("DungeonUnlock") end)
        task.wait(0.3) SP_ClickYes()
        SP_DungeonStep=1 SP_DungeonCollected={} Library:Notify("🌿 Dungeon Quest bắt đầu!",3) return
    end
    if step>6 then
        SP_ForceTP("Dungeon") SP_WaitChar(10) task.wait(1)
        pcall(function() RS.RemoteEvents.QuestAccept:FireServer("DungeonUnlock") end)
        Toggles.DungeonQuest:SetValue(false) SP_DungeonStep=0 SP_DungeonCollected={}
        Library:Notify("🌿 Dungeon Quest hoàn thành!",5) return
    end
    local isl=SP_DungeonPieceIslands[step] if not isl then SP_DungeonStep=step+1 return end
    if SP_DungeonCollected[step] then SP_DungeonStep=step+1 return end
    Library:Notify("🌿 Dungeon #"..step.." → "..isl,2)
    SP_ForceTP(isl) SP_WaitChar(8) task.wait(0.5)
    local p=SP_FindDungeonPiece(isl) if not p then task.wait(1) p=SP_FindDungeonPiece(isl) end
    if not p then task.wait(1) p=SP_FindDungeonPiece(isl) end
    if p and SP_PieceHasPrompt(p) then SP_CollectPiece(p)
    else Library:Notify("🌿 Dungeon: Không tìm mảnh #"..step,2) end
    SP_DungeonCollected[step]=true SP_DungeonStep=step+1
end
local function SP_DoHogyokuTick()
    if not Toggles.HogyokuQuest.Value then return end
    local step=SP_HogyokuStep
    if step==0 then SP_ForceTP("HuecoMundo") SP_HogyokuStep=0.5 return end
    if step==0.5 then
        SP_WaitChar(10) task.wait(0.5)
        ...
        if not npc then Library:Notify("🌿 Hogyoku: Không tìm NPC!",2) SP_HogyokuStep=0 return end
        local pos local p=npc:FindFirstChild("HumanoidRootPart",true) or npc:FindFirstChild("Head",true)
        if p and p:IsA("BasePart") then pos=p.Position end
        if not pos then for _,d in ipairs(npc:GetDescendants()) do if d:IsA("BasePart") then pos=d.Position break end end end
        if not pos then SP_HogyokuStep=0 return end
        local hrp=Plr.Character and Plr.Character:FindFirstChild("HumanoidRootPart")
        if hrp then hrp.CFrame=CFrame.new(pos+Vector3.new(0,0,3)) end task.wait(0.3)
        ...
        task.wait(0.4) SP_ClickYes() task.wait(0.3) SP_ClickYes()
        SP_HogyokuStep=1 SP_HogyokuCollected={} task.wait(1)
        Library:Notify("🌿 Hogyoku: Nhận quest!",3) return
    end
    if step>6 then
        SP_ForceTP("HuecoMundo") SP_WaitChar(10) task.wait(0.5)
        local npc=workspace:FindFirstChild("HogyokuQuestNPC",true)
        if npc then
            local pos local p=npc:FindFirstChild("HumanoidRootPart",true) or npc:FindFirstChild("Head",true)
            if p and p:IsA("BasePart") then pos=p.Position end
            if pos then local hrp=Plr.Character and Plr.Character:FindFirstChild("HumanoidRootPart") if hrp then hrp.CFrame=CFrame.new(pos+Vector3.new(0,0,3)) end end
            task.wait(0.3)
            for _,d in ipairs(npc:GetDescendants()) do if d:IsA("ProximityPrompt") then pcall(function() fireproximityprompt(d) end) end end
            task.wait(0.3) SP_ClickYes() task.wait(0.3) SP_ClickYes()
        end
        Toggles.HogyokuQuest:SetValue(false) SP_HogyokuStep=0 SP_HogyokuCollected={}
        Library:Notify("🌿 Hogyoku: Hoàn thành! Soul Society mở!",5) return
    end
    local isl=SP_HogyokuFragmentIslands[step] if not isl then SP_HogyokuStep=step+1 return end
    if SP_HogyokuCollected[step] then SP_HogyokuStep=step+1 return end
    SP_ForceTP(isl) SP_WaitChar(8) task.wait(0.5)
    local prompt=SP_FindHogyokuFrag(step)
    if not prompt then task.wait(1) prompt=SP_FindHogyokuFrag(step) end
    if not prompt then task.wait(1) prompt=SP_FindHogyokuFrag(step) end
    if prompt then
        local hrp=Plr.Character and Plr.Character:FindFirstChild("HumanoidRootPart")
        local par=prompt.Parent
        if par and hrp then local pos=par:IsA("BasePart") and par.Position or nil if not pos then pcall(function() pos=par:GetPivot().Position end) end if pos then hrp.CFrame=CFrame.new(pos+Vector3.new(0,2,0)) end end
        task.wait(0.1) pcall(function() fireproximityprompt(prompt) end) task.wait(0.15) pcall(function() fireproximityprompt(prompt) end)
        SP_HogyokuCollected[step]=true SP_HogyokuStep=step+1
        Library:Notify("🌿 Hogyoku: Mảnh #"..step.." ✓",2)
    else
        Library:Notify("🌿 Hogyoku: Không tìm mảnh #"..step,2)
    end
end
task.spawn(function()
    while getgenv().celina_Running do
        task.wait(0.3)
        if Toggles.DungeonQuest.Value then pcall(SP_DoDungeonTick) end
        if Toggles.HogyokuQuest.Value then pcall(SP_DoHogyokuTick) end
    end
end)
...
Library:Notify("🌿 Centella asiatica loaded!", 3)
Library:Notify("by VanTuanDEV — Sailor Piece", 4)
